...
func newCacher(name string, lang string, plat platform.Platform, get bool) *cacher {
	return &cacher{name: name + ".md", lang: lang, plat: plat, get: get}
}
...
func Find(name string, lang string, plat platform.Platform, get bool) (*os.File, string, platform.Platform) {
	cacher := newCacher(name, lang, platform.GetPlatform(plat), get)
...
func Remove(name string, lang string, plat platform.Platform, exit bool) {
	cacher := newCacher(name, lang, platform.GetPlatform(plat), false)
	cacher.remove(exit)
}
